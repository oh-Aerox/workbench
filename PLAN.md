...
- [ ] **源码 `TODO/FIXME` 作为计划面板的补充分区**
  - 曾评估过：本机现成 371 处，但 352 处集中在 eshop 一个项目
    （分散在 manage/inc/static 等自有目录，是长期遗留标记），其余项目各 0–5 处
  - **代价**：应用的读取范围会从三个 agent 数据目录扩大到用户全部项目的源码树。
    仍是只读、不违反需求 2，但要加忽略规则（node_modules/.git/dist/target 等）
    和扫描缓存，否则大仓库会卡
  - 当前决策：**不做**。计划面板只读 agent 自己的记录
